...
Peek 把这个过程改成了浮层阅读。帖子和评论放在同一个窗口里，左右两栏各自滚动；关掉浮层以后，页面仍然停在打开前的位置。
如果已经进入 `/status/...` 帖子详情页，插件不会重复拦截普通帖子和回复，继续保留 X 原生交互；但点击回复里的引用帖卡片时，会单独打开这条引用帖的浮层。

## 02 现在能做什么

本次更新内容见 [2026-09-25 更新日志](CHANGELOG.md)。
...
- **原生收藏分类**：收藏时可以选择 X 原生收藏夹，也可仅保存到所有书签；已收藏的帖子支持再次选择分类。文件夹需要相应 X 账号权限，读取失败时仍可普通收藏。新建和管理文件夹通过 X 原生页面完成。
...
以后更新代码后，重新执行 `npm run build:extension`，再到扩展管理页点击“重新加载”，并刷新已打开的 X 标签页，让页面桥接脚本和浮层一起更新。
...
当前测试会检查帖子链接、引用帖目标与媒体归属解析、详情页避让、回复上下文、评论分页、图片和视频识别、网站卡片、X 长文、用户数据、扩展权限、图标与播放器本地打包，以及预览站点构建。关注专项测试还覆盖真实桥接消息入口、请求格式、失败恢复、重复点击和私密账号待批准状态。
